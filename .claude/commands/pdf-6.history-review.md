...
> **인증 정책**: 히스토리/리뷰/통계 API 및 페이지는 모두 **로그인 필수** (🔒).
> 모든 API에 `requireSession` 적용, 본인 데이터만 조회/수정 가능.

1. 변환 이력 저장 (in-memory store)
2. 히스토리 목록 / 상세 API + UI (🔒 로그인 필수)
3. 변환 리뷰 입력 (섹션별 정확도 점수 + 코멘트) (🔒 로그인 필수)
4. 성능 통계 대시보드 (어떤 필드가 자주 틀리는지) (🔒 로그인 필수)
